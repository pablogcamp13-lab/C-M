...
type LinkRecord = { id:string; companyId:string; campaignId:string; companyName:string; campaignName:string; createdAt:string; expiresAt:string; revokedAt:string|null };
...
export const PublicDashboardLinksModal:React.FC<{onClose:()=>void}>=({onClose})=>{
  ...
  return <Modal open title="Compartir dashboard con cliente" description="Enlace público de solo lectura, limitado a una empresa y campaña." onClose={onClose} size="lg" footer={<Button variant="secondary" onClick={onClose}>Cerrar</Button>}>
    <div className="space-y-5">
      ...
      <Button leadingIcon={<Link2/>} disabled={!companyId||!campaignId} loading={busy} onClick={create}>Generar enlace</Button>
      ...
      <section aria-label="Enlaces generados"><h3 className="mb-2 font-semibold">Enlaces generados</h3>{links.length?<div className="max-h-56 space-y-2 overflow-y-auto">{links.map(link=>{const active=!link.revokedAt&&Date.parse(link.expiresAt)>Date.now();return <div key={link.id} className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-[var(--cm-border)] p-3 text-sm"><span><strong>{link.companyName} / {link.campaignName}</strong><small className="block text-[var(--cm-text-secondary)]">{link.revokedAt?'Revocado':active?`Vence ${new Date(link.expiresAt).toLocaleDateString('es-PE')}`:'Caducado'}</small></span>{active&&<Button size="sm" variant="danger" leadingIcon={<ShieldOff/>} disabled={busy} onClick={()=>revoke(link.id)}>Revocar</Button>}</div>;})}</div>:<p className="text-sm text-[var(--cm-text-secondary)]">Aún no hay enlaces compartidos.</p>}</section>
    </div>
  </Modal>;
};
