...
export const AdvisorHomeView: React.FC = () => {
  ...
  const ownEvaluations = dashboardEvaluations.filter(e=>!currentUser.advisorId || e.advisorId===currentUser.advisorId);
  const ownPlans = actionPlans.filter(plan=>plan.advisorId===currentUser.advisorId);
  const quality = ownEvaluations.filter(e=>e.evaluationType==='QUALITY').sort((a,b)=>b.date.localeCompare(a.date));
