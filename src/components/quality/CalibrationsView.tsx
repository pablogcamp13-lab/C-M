...
export const CalibrationsView: React.FC = () => {
  ...
  const transition = async (status: string) => {
    ...
    try {
      ...
      if (status === 'PROGRAMADA') try { await calibrationsApi.invite(selected.id); } catch { setError('La sesión fue programada; revisa el envío de invitaciones.'); }
      await reload();
    } catch (e) {
  ...
  const invite = async () => { if (!selected) return; setBusy(true); setError(''); try { await calibrationsApi.invite(selected.id); await reload(); } catch (e) { setError(e instanceof Error ? e.message : 'No fue posible enviar invitaciones.'); } finally { setBusy(false); } };
  const remove = async () => {
    ...
    <main className="cm-workspace flex-1 overflow-y-auto p-4 lg:p-6">
      <div className="mx-auto max-w-[1600px] space-y-4">
        ...
        <section className="grid min-h-[620px] gap-4 xl:grid-cols-[minmax(520px,0.9fr)_minmax(620px,1.1fr)]">
          ...
          <div className="max-h-[650px] overflow-auto p-4">{tab==='MATRIZ'&&<Matrix item={selected} users={users}/>} {tab==='PARTICIPANTES'&&<Participants item={selected} users={users}/>} {tab==='INFORMACION'&&<GeneralInfo item={selected} users={users}/>}<div className="mt-4 flex flex-wrap gap-2">{manager&&norm(selected.status)==='BORRADOR'&&<Action text="Programar" run={()=>transition('PROGRAMADA')} busy={busy}/>} {manager&&['PROGRAMADA','EN_VIVO'].includes(norm(selected.status))&&<Action text="Enviar / reintentar invitaciones" run={invite} busy={busy}/>} {manager&&norm(selected.status)==='PROGRAMADA'&&<Action text="Iniciar" run={()=>transition('EN_VIVO')} busy={busy}/>} {manager&&norm(selected.status)==='EN_VIVO'&&<Action text="Finalizar" run={()=>transition('FINALIZADA')} busy={busy}/>} {manager&&norm(selected.status)==='FINALIZADA'&&<Action text="Cerrar y publicar" run={()=>transition('CERRADA')} busy={busy}/>} {mayRespond&&<Action text="Responder en vivo" run={()=>setResponding(selected)} busy={busy}/>} {manager&&(!['CERRADA','ANULADA'].includes(norm(selected.status))||admin)&&<button onClick={()=>setEditing(selected)} className="cm-button-secondary px-3 py-2"><Edit3 className="h-4 w-4"/>Editar</button>} {closed&&<button onClick={exportCsv} className="cm-button-secondary px-3 py-2"><Download className="h-4 w-4"/>CSV</button>} {manager&&!['CERRADA','ANULADA'].includes(norm(selected.status))&&<button onClick={()=>void transition('ANULADA')} className="cm-button-secondary px-3 py-2">Anular</button>} {admin&&<button onClick={()=>void remove()} className="cm-button-secondary px-3 py-2 text-red-300"><Trash2 className="h-4 w-4"/></button>}</div></div></article>}
        </section>}
      </div>
      {createOpen && (
        <Form
