...
import { CampaignQualityModal } from "./CampaignQualityModal";
import {
  Settings,
  Save,
  RotateCcw,
...
export const AdminSettingsView: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    "users" | "methodology" | "campaigns" | "database"
  >("users");

  // Methodology form state
  const [connectWeight, setConnectWeight] = useState<number>(
    config.weights.CONECTAR || config.weights.connect || 30,
  );
  const [clarifyWeight, setClarifyWeight] = useState<number>(
    config.weights.CLARIFICAR || config.weights.clarify || 35,
  );
  const [convertWeight, setConvertWeight] = useState<number>(
    config.weights.CONVERTIR || config.weights.convert || 35,
  );
  ...
  const [highGapMax, setHighGapMax] = useState<number>(
    config.priorityThresholds.highGapMax,
  );
  const [mediumGapMax, setMediumGapMax] = useState<number>(
    config.priorityThresholds.mediumGapMax,
  );
  const [expectedMax, setExpectedMax] = useState<number>(
    config.priorityThresholds.expectedMax,
  );
  ...
  const [campProducts, setCampProducts] = useState<string>("");
  const [managedCampaignId, setManagedCampaignId] = useState<string | null>(
    null,
  );
  ...
  const handleAutoCreateAdvisorUsers = async () => {
    try {
      ...
      showNotification(
        count > 0
          ? `Se crearon automáticamente ${count} cuentas de usuario para los asesores.`
          : "Todos los asesores ya cuentan con un usuario asignado.",
      );
    } catch (error) {
      alert(
        error instanceof Error
          ? error.message
          : "No fue posible crear las cuentas.",
      );
    }
  };
  ...
  const handleResetPassword = async (user: User) => {
    if (
      !window.confirm(`¿Restablecer la contraseña de ${user.name} a 12345678?`)
    )
      return;
    try {
      ...
      showNotification(
        "Contraseña restablecida. Se solicitará el cambio al ingresar.",
      );
    } catch (error) {
      alert(
        error instanceof Error
  ...
  const handleCreateCampaign = (e: React.FormEvent) => {
    ...
    addCampaign({
      ...
      products: campProducts
        ...
        .filter(Boolean),
      description: campDescription.trim() || "Campaña activa",
      qualityGuidelines: [],
    });
  ...
  const advisorsWithoutUser = advisors.filter(
    (adv) => !users.some((u) => u.role === "ASESOR" && u.advisorId === adv.id),
  );
  ...
  return (
    <div className="cm-workspace cm-admin flex-1 overflow-y-auto bg-[#F7F8FA] text-[#031E3C]">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white border border-[#E5E8EC] rounded-xl p-5 shadow-2xs">
          <div>
            ...
            <p className="text-xs text-[#667085] mt-0.5">
              Crea usuarios para asesores, supervisores y administradores, y
              ajusta las ponderaciones metodológicas 3C.
            </p>
          </div>
              ...
              <div className="flex items-center gap-2">
                {advisorsWithoutUser.length > 0 && (
                  ...
                  >
                    ...
                    <span>
                      Crear Cuentas a Asesores ({advisorsWithoutUser.length})
                    </span>
                  </button>
                )}
                ...
                <table className="w-full text-xs text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-[#E5E8EC]">
                    {filteredUsersList.map((user) => {
                      const linkedAdvisor = advisors.find(
                        (a) => a.id === user.advisorId,
                      );
                      const isCurrent = currentUser?.id === user.id;
                      const displayUsername =
                        user.username || formatAdvisorUsername(user.name);
                      const displayPassword = user.mustChangePassword
                        ? "12345678 · cambio pendiente"
                        : "Contraseña configurada";

                      return (
                        <tr
                          key={user.id}
                          className={`hover:bg-[#F7F8FA]/70 transition-colors ${isCurrent ? "bg-orange-50/40" : ""}`}
                        >
                          {/* Usuario Info */}
                            ...
                            <div className="flex items-center gap-2.5">
                              ...
                              <div>
                                <div className="flex items-center gap-1.5">
                                  <span className="font-bold text-[#031E3C]">
                                    {user.name}
                                  </span>
                                  {isCurrent && (
                          ...
                          <td className="py-3 px-4">
                            <div className="space-y-0.5 font-mono text-[11px]">
                              <div className="flex items-center gap-1 text-[#031E3C]">
                                <span className="text-[10px] text-[#667085] font-sans font-semibold">
                                  Usuario:
                                </span>
                            ...
                            {user.role === "ASESOR" ? (
                              linkedAdvisor ? (
                                <div className="flex items-center gap-1 text-emerald-900 font-semibold text-xs">
                                  ...
                                  <span>
                                    Asesor: {linkedAdvisor.name} (
                                    {linkedAdvisor.employeeCode})
                                  </span>
                                </div>
                              ) : (
                          ...
                          <td className="py-3 px-4">
                            <span
                              className={`cm-badge ${user.status === "ACTIVO" ? "cm-badge--success" : "text-[var(--cm-text-secondary)]"}`}
                            >
                              {user.status}
                            </span>
                          </td>
                          ...
                          <td className="py-3 px-4 text-right space-x-1">
                            ...
                            <button
                              ...
                              className="text-[10px] px-2 py-1 text-sky-700 hover:bg-sky-50 rounded"
          ...
          <form onSubmit={handleSaveConfig} className="space-y-6">
            ...
            <div className="bg-white border border-[#E5E8EC] rounded-xl p-5 shadow-2xs space-y-4">
              <div className="flex items-center justify-between border-b border-[#E5E8EC] pb-3">
                <div>
                  ...
                  <p className="text-[11px] text-[#667085]">
                    Define el peso de cada dimensión en el cálculo de la nota
                    final de la llamada.
                  </p>
                </div>
                <div
                  className={`px-3 py-1 rounded-full text-xs font-bold ${isWeightValid ? "bg-emerald-100 text-emerald-800" : "bg-rose-100 text-rose-800"}`}
                >
                  Suma: {totalWeights}%{" "}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="bg-sky-50/60 border border-sky-200 p-3.5 rounded-xl">
                  <label className="block font-bold text-sky-900 text-xs mb-1">
                    C1: CONECTAR (%)
                  </label>
                  <input
                    type="number"
                ...
                <div className="bg-amber-50/60 border border-amber-200 p-3.5 rounded-xl">
                  ...
                  <span className="text-[10px] text-amber-700 mt-1 block">
                    Dominio, Organización, BiPay
                  </span>
                </div>

                <div className="bg-emerald-50/60 border border-emerald-200 p-3.5 rounded-xl">
                  <label className="block font-bold text-emerald-900 text-xs mb-1">
                    C3: CONVERTIR (%)
                  </label>
                  <input
                    type="number"
            ...
            <div className="bg-white border border-[#E5E8EC] rounded-xl p-5 shadow-2xs space-y-4">
              <div className="border-b border-[#E5E8EC] pb-3">
                ...
                <p className="text-[11px] text-[#667085]">
                  Porcentaje asignado a cada nivel de cumplimiento en los ítems
                  de calidad.
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
                ...
                <div className="bg-[#F7F8FA] border border-[#E5E8EC] p-3 rounded-lg">
                  <label className="block font-bold text-sky-700 mb-1">
                    Nivel 3 (Parcial alto)
                  </label>
                  <input
                    type="number"
                    value={level3Pct}
        ...
        {activeTab === "campaigns" && (
          <div className="space-y-4">
            <div className="flex items-center justify-between bg-white border border-[#E5E8EC] rounded-xl p-3.5 shadow-2xs">
              <span className="font-bold text-xs text-[#031E3C]">
                Campañas y lineamientos de Calidad
              </span>
              <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {campaigns.map((c) => (
                <div
                  key={c.id}
                  className="bg-white border border-[#E5E8EC] rounded-xl p-4 shadow-2xs"
                >
                  ...
                  <h4 className="font-bold text-sm text-[#031E3C] mt-2">
                    {c.name}
                  </h4>
                  ...
                  <div className="mt-3 pt-3 border-t border-[#E5E8EC] flex items-center gap-1 flex-wrap">
                    {c.products.map((p) => (
                      <span
                        key={p}
                        className="text-[10px] font-semibold bg-[#F7F8FA] border border-[#E5E8EC] px-2 py-0.5 rounded text-[#031E3C]"
                      >
                        {p}
                      </span>
                    ))}
                  ...
                  <button
                    type="button"
                    onClick={() => setManagedCampaignId(c.id)}
                    className="mt-3 w-full rounded-lg border border-cyan-500/50 px-3 py-2 text-xs font-bold text-cyan-300"
                  >
                    Configurar campaña y lineamientos (
                    {c.qualityGuidelines?.length || 0})
                  </button>
                </div>
              ))}
            </div>
          ...
          <div className="fixed inset-0 z-[200] bg-slate-950/60 backdrop-blur-sm flex items-center justify-center overflow-y-auto p-4">
            <div className="cm-modal my-auto max-w-md w-full p-6">
              ...
              <p className="text-xs text-[#667085] mb-4">
                Crea credenciales para que los asesores, supervisores o
              ...
              <form onSubmit={handleCreateUser} className="space-y-3.5 text-xs">
                ...
                <div>
                  ...
                  >
                    <option value="ASESOR">
                      ASESOR (Ve sus evaluaciones, intervenciones y planes de
                      acción)
                    </option>
                    <option value="SUPERVISOR">
                      SUPERVISOR (Gestiona su equipo)
                    </option>
                    <option value="FORMADOR">
                      FORMADOR (Asigna y ejecuta intervenciones)
                    </option>
      ...
      {isCampaignModalOpen && (
        <div className="fixed inset-0 z-50 bg-[#031E3C]/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="cm-modal max-w-md w-full p-6">
            <h3 className="font-bold text-base text-[#031E3C] mb-3">
              Crear Nueva Campaña
            </h3>
            <form onSubmit={handleCreateCampaign} className="space-y-3 text-xs">
              ...
              <div>
                ...
                <input
                  value={campProducts}
                  onChange={(e) => setCampProducts(e.target.value)}
                  placeholder="Ej: Migraciones, Portabilidad"
                  className="w-full bg-[#F7F8FA] border border-[#E5E8EC] rounded-lg p-2"
                />
      ...
      {managedCampaignId &&
        campaigns.find((campaign) => campaign.id === managedCampaignId) &&
        createPortal(
          <CampaignQualityModal
            campaign={campaigns.find(
              (campaign) => campaign.id === managedCampaignId,
            )!}
            onClose={() => setManagedCampaignId(null)}
            onSave={(data) => {
              updateCampaign(managedCampaignId, data);
              showNotification("Campaña y lineamientos actualizados.");
            }}
          />,
          document.body,
        )}
