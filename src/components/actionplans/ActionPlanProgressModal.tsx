    ...
    <div className="cm-modal flex max-h-[92vh] w-full max-w-5xl flex-col overflow-hidden">
      ...
      <main className="min-h-0 overflow-y-auto p-5 sm:p-6">
        ...
        <div className="grid gap-4 lg:grid-cols-4">
          <section className="min-w-0 rounded-xl border border-[var(--cm-border)] bg-[var(--cm-surface-elevated)] p-4 lg:col-span-3" aria-label="Tendencia de SPH">
            ...
            {trend.some(point => point.sph !== null) ? <div className="h-72 w-full sm:h-80"><ResponsiveContainer width="100%" height="100%"><LineChart data={trend} margin={{ top: 14, right: 20, left: -18, bottom: 8 }} accessibilityLayer><CartesianGrid stroke="rgba(143,167,192,.18)" vertical={false}/><XAxis dataKey="stage" tick={{ fill: '#b9cce0', fontSize: 11 }} tickLine={false} axisLine={false}/><YAxis domain={[0, 'auto']} tick={{ fill: '#8fa7c0', fontSize: 10 }} tickLine={false} axisLine={false} tickFormatter={value => Number(value).toFixed(2)}/><Tooltip content={({ active, payload }) => active && payload?.[0] ? <div className="cm-chart-tooltip"><b>{payload[0].payload.stage}</b><span>SPH: {Number(payload[0].payload.sph).toFixed(2)}</span><span>Fecha: {dateLabel(payload[0].payload)}</span><span>{payload[0].payload.count} con dato</span></div> : null}/><Line type="monotone" dataKey="sph" connectNulls stroke="#1fd6ff" strokeWidth={3} dot={{ r: 5, fill: '#0d2037', stroke: '#1fd6ff', strokeWidth: 2 }} activeDot={{ r: 7 }}/></LineChart></ResponsiveContainer></div> : <p className="grid h-72 place-items-center text-sm text-[var(--cm-text-secondary)] sm:h-80">Aún no hay valores de SPH registrados.</p>}
          ...
          <section className="min-w-0 rounded-xl border border-[var(--cm-border)] bg-[var(--cm-surface-elevated)] p-3 lg:col-span-1" aria-label="Observaciones de asesores del plan">
            <h3 className="text-sm font-bold">Observaciones</h3><p className="mb-2 text-[10px] text-[var(--cm-text-secondary)]">{selectedAdvisorId ? 'Detalle completo del asesor.' : 'Pasa el cursor para leer el comentario completo.'}</p>
            <div className="max-h-80 space-y-1.5 overflow-y-auto pr-1">{observations.map(item => <div key={item.advisorId} className={`rounded-lg border border-[var(--cm-border)] bg-[var(--cm-surface)] p-2 text-[11px] ${selectedAdvisorId ? '' : 'flex min-w-0 items-center gap-2'}`}>
              <b className={`${selectedAdvisorId ? 'mb-1 block' : 'w-24 shrink-0 truncate'} text-cyan-300`} title={item.advisorName}>{item.advisorName}</b>
              {selectedAdvisorId ? <p className="whitespace-pre-wrap break-words leading-relaxed text-[var(--cm-text-secondary)]">{item.text}</p> : <div className="group relative min-w-0 flex-1" title={item.text} tabIndex={0}><p className="truncate text-[var(--cm-text-secondary)]">{item.text}</p><div role="tooltip" className="pointer-events-none invisible absolute bottom-[calc(100%+.4rem)] right-0 z-20 w-max max-w-[min(32rem,75vw)] rounded-lg border border-cyan-400/30 bg-slate-950 px-3 py-2 text-left leading-relaxed text-slate-100 opacity-0 shadow-2xl transition group-hover:visible group-hover:opacity-100 group-focus:visible group-focus:opacity-100">{item.text}</div></div>}
            </div>)}</div>
          </section>
        </div>
      </main>
