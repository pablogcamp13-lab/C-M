...
import { applySharedMetricDate, inheritedMetricDates, sharedMetricDate } from './actionPlanMetricDates';
import { 
  ListTodo, 
  Plus, 
  CheckCircle2, 
...
export const ActionPlansManager: React.FC<ActionPlansManagerProps> = ({
  ...
  const sharedDateFields = (rows: ActionPlanAdvisorMetric[], setRows: React.Dispatch<React.SetStateAction<ActionPlanAdvisorMetric[]>>, readOnly = false) => <div className="rounded-lg border border-[#E5E8EC] bg-[#F6F7F9] p-3">
    <p className="font-semibold text-[#031E3C]">Fechas de medición SPH</p><p className="mb-2 text-[11px] text-[#667085]">Una fecha por etapa; se aplicará a todos los asesores del plan.</p>
    <div className="grid gap-2 sm:grid-cols-3">{([
      ['sphInitialDate', 'inicial'], ['sphRetrainingDate', 'reentrenamiento'], ['sphUpdatedDate', 'actualizado'],
    ] as const).map(([key, label]) => {
      const distinctDates = new Set(rows.map(row => row[key] || ''));
      return <label key={key} className="block">Fecha SPH {label}<input type="date" disabled={readOnly} value={sharedMetricDate(rows, key)} onChange={e => setRows(previous => applySharedMetricDate(previous, key, e.target.value))} className="mt-1 w-full rounded-lg border border-[#E5E8EC] bg-white px-3 py-2" />{distinctDates.size > 1 && <small className="text-amber-700">Fechas distintas; elige una para unificarlas.</small>}</label>;
    })}</div>
  </div>;
  const metricFields = (rows: ActionPlanAdvisorMetric[], setRows: React.Dispatch<React.SetStateAction<ActionPlanAdvisorMetric[]>>, readOnly = false) => rows.map((row, index) => (
    <div key={row.advisorId} className="rounded-lg border border-[#E5E8EC] bg-[#F6F7F9] p-3 space-y-2">
      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
        {([
          ['sphInitial', 'inicial'],
          ['sphRetraining', 'reentrenamiento'],
          ['sphUpdated', 'actualizado'],
        ] as const).map(([valueKey, label]) => <div key={valueKey} className="space-y-2">
  ...
  return (
    <div className="cm-workspace cm-plans flex-1 flex flex-col min-h-0 overflow-y-auto bg-[#F6F7F9]">
            ...
            <form onSubmit={handleCreatePlan} className="space-y-4 pt-4 text-xs">
              ...
              <div>
                ...
                <div className="mt-2 max-h-36 overflow-y-auto rounded-lg border border-[#E5E8EC] bg-[#F6F7F9] p-2 space-y-1">
                  {eligibleAdvisors.filter(a => a.name.toLowerCase().includes(advisorSearch.toLowerCase())).map(a => <label key={a.id} className="flex items-center gap-2 p-1"><input type="checkbox" checked={advisorMetrics.some(row => row.advisorId === a.id)} disabled={a.id === initialEvaluationForPlan?.advisorId} onChange={e => setAdvisorMetrics(prev => e.target.checked ? [...prev, { advisorId: a.id, sphInitial: null, sphUpdated: null, sphRetraining: null, ...inheritedMetricDates(prev), followUpType: 'SEGUIMIENTO_FEEDBACK', observations: '' }] : prev.filter(row => row.advisorId !== a.id))} />{a.name}</label>)}
                </div>
                <p className="mt-1 text-[#667085]">El grupo debe compartir campaña, operación y supervisor.</p>
              </div>
              {advisorMetrics.length > 0 && <div className="space-y-2">{sharedDateFields(advisorMetrics, setAdvisorMetrics)}<p className="font-semibold text-[#031E3C]">Seguimiento por asesor</p>{metricFields(advisorMetrics, setAdvisorMetrics)}</div>}
      ...
      {adminEditPlan && isAdmin && createPortal(<div className="fixed inset-0 z-[110] flex items-center justify-center bg-[#031E3C]/60 p-4 backdrop-blur-xs"><div className="cm-modal max-w-3xl w-full max-h-[92vh] overflow-y-auto p-5"><div className="flex justify-between items-center border-b border-[#E5E8EC] pb-3"><div><p className="text-[10px] font-bold uppercase text-[#FF6B00]">Administración</p><h3 className="font-bold text-[#031E3C]">Editar plan de acción completo</h3></div><button type="button" onClick={() => setAdminEditPlan(null)} aria-label="Cerrar"><X className="w-5 h-5" /></button></div><form onSubmit={handleSaveAdminPlan} className="space-y-3 pt-4 text-xs">
        <div><label className="block font-semibold text-[#031E3C]">Grupo de asesores</label><div className="mt-1 max-h-28 overflow-y-auto rounded-lg border border-[#E5E8EC] bg-[#F6F7F9] p-2">{adminEligibleAdvisors.map(a => <label key={a.id} className="flex items-center gap-2 p-1"><input type="checkbox" checked={adminEditAdvisorIds.includes(a.id)} onChange={e => { setAdminEditAdvisorIds(ids => e.target.checked ? [...ids, a.id] : ids.filter(id => id !== a.id)); setAdminEditMetrics(rows => e.target.checked ? [...rows, { advisorId: a.id, sphInitial: null, sphUpdated: null, sphRetraining: null, ...inheritedMetricDates(rows), followUpType: 'SEGUIMIENTO_FEEDBACK', observations: '' }] : rows.filter(row => row.advisorId !== a.id)); }} />{a.name}</label>)}</div></div>
        ...
        {adminEditMetrics.length > 0 && <div className="space-y-2">{sharedDateFields(adminEditMetrics, setAdminEditMetrics)}<p className="font-semibold text-[#031E3C]">SPH y seguimiento por asesor</p>{metricFields(adminEditMetrics, setAdminEditMetrics)}</div>}
