...
export const Navbar: React.FC<{ onOpenNewEvaluation: () => void }> = ({ onOpenNewEvaluation }) => {
  ...
  const overduePlans = actionPlans.filter(plan => plan.status === 'VENCIDO' && (plan.advisorIds?.length ? plan.advisorIds : [plan.advisorId]).some(id => scopedAdvisorIds.has(id))).length;
