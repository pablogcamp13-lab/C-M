  ...
  return <header className="cm-navbar">
    <div className="cm-navbar__inner">
      ...
      <div className="ml-auto flex items-center gap-2">
        ...
        {canCreateEvaluation && <button onClick={onOpenNewEvaluation} className="cm-button-primary hidden px-3 py-2 text-xs md:inline-flex"><Plus className="h-4 w-4" />Nueva evaluación</button>}
    ...
    {open && <nav className="cm-navbar__mobile lg:hidden" aria-label="Navegación móvil">{visibleNavigation.map(item => <button key={item.section} onClick={() => { setCurrentSection(item.section); setOpen(false); }} className="cm-navbar__mobile-link" aria-current={currentSection === item.section ? 'page' : undefined}>{item.label}</button>)}{canCreateEvaluation && <button onClick={onOpenNewEvaluation} className="cm-button-primary mt-2 w-full px-3 py-2 text-left text-sm">+ Nueva evaluación</button>}</nav>}
  </header>;
};
