          ...
          {activeTab === '3C' && (
            <>
              ...
              <div>
                ...
                {memorandums.map(memo=><details key={memo.id} className="mb-2 rounded-xl border border-slate-200 bg-white p-3 text-xs text-slate-800"><summary className="cursor-pointer font-bold">{memo.number} · {memo.createdAt.slice(0,10)} · {memo.subject} · {memo.status==='ANULADO'?'Anulado':'Emitido'}</summary><div className="mt-3 space-y-2"><p><b>Alerta:</b> {memo.alertTitle}</p><p className="whitespace-pre-wrap"><b>Hechos:</b> {memo.content}</p><p className="whitespace-pre-wrap"><b>Acción requerida:</b> {memo.actionRequired}</p><p><b>Emitido por:</b> {memo.issuerName}</p>{Boolean(memo.attachments?.length)&&<div><b>Anexos:</b>{memo.attachments?.map(file=><button key={file.id} type="button" className="mt-1 block text-blue-700 underline" onClick={()=>void memorandumsApi.downloadAttachment(memo.id,file).catch(()=>setMemoError('No se pudo descargar el anexo.'))}>{file.name}</button>)}</div>}{memo.status==='ANULADO'&&<p className="text-rose-600"><b>Motivo de anulación:</b> {memo.annulReason}</p>}</div></details>)}
