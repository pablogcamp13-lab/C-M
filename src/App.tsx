...
const EvaluationModulePicker: React.FC<{ onSelect: (module: 'QUALITY' | 'D3C') => void; onClose: () => void }> = ({ onSelect, onClose }) => {
  ...
  const options = [{ id: 'QUALITY' as const, title: 'Calidad', description: 'PUE Migraciones · cumplimiento y estándar', icon: <ShieldCheck className="h-9 w-9" /> }, { id: 'D3C' as const, title: 'Mejora Continua D+3C', description: 'Diagnóstico comercial y plan de mejora', icon: <TrendingUp className="h-9 w-9" /> }];
