...
const MainLayout: React.FC = () => {
  ...
  const handleOpenNewEvaluation = async (advisor?: Advisor) => {
    ...
    try { const refreshed = await refreshRepository(); latest = { advisors: refreshed.advisors, operations: refreshed.operations || [] }; } catch {}
    const freshAdvisor = advisor ? latest.advisors.find(item => item.id === advisor.id) : undefined;
    setPreselectedAdvisorForEval(freshAdvisor || null);
    const operation=latest.operations.find(item=>item.id===freshAdvisor?.operationId && item.status==='ACTIVA' && !item.legacy);
    setNewEvaluationCompanyId(operation?.companyId || null);
    setNewEvaluationOperationId(operation?.id || null);
    setNewEvaluationCampaignId(operation?.campaignId || null);
