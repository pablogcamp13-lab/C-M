...
async function startServer() {
  ...
  const loadCalibrations=async()=>{let rows=calibrationRows();try{const remote=googleStorage.enabled?await googleStorage.loadCalibrations():[];if(remote.length){rows=remote;remote.forEach(persistCalibration);}}catch{}return rows;};
