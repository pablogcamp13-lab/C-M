...
CREATE TABLE IF NOT EXISTS memorandums (id TEXT PRIMARY KEY, alert_id TEXT NOT NULL UNIQUE, advisor_id TEXT NOT NULL, status TEXT NOT NULL, data_json TEXT NOT NULL, created_at TEXT NOT NULL, updated_at TEXT NOT NULL);
CREATE INDEX IF NOT EXISTS idx_memorandums_advisor ON memorandums(advisor_id, created_at DESC);
...
async function startServer() {
  ...
  const publicLinkKey=(id:string)=>`public_dashboard_link:${id}`;
  const listPublicLinks=async():Promise<any[]>=>supabaseStorage.enabled?supabaseStorage.listPublicDashboardLinks():(db.prepare("SELECT payload_json FROM app_state WHERE id LIKE 'public_dashboard_link:%' ORDER BY updated_at DESC").all() as any[]).map(row=>JSON.parse(row.payload_json));
  const getPublicLink=async(id:string):Promise<any>=>supabaseStorage.enabled?supabaseStorage.getPublicDashboardLink(id):JSON.parse(String((db.prepare('SELECT payload_json FROM app_state WHERE id=?').get(publicLinkKey(id)) as any)?.payload_json||'null'));
  const savePublicLink=async(link:any)=>{if(supabaseStorage.enabled)await supabaseStorage.savePublicDashboardLink(link);else db.prepare('INSERT INTO app_state(id,payload_json,updated_at) VALUES(?,?,?) ON CONFLICT(id) DO UPDATE SET payload_json=excluded.payload_json,updated_at=excluded.updated_at').run(publicLinkKey(link.id),JSON.stringify(link),new Date().toISOString());};
  const publicLinkSummary=(link:any)=>({id:link.id,companyId:link.companyId,campaignId:link.campaignId,companyName:link.companyName,campaignName:link.campaignName,dashboardType:link.dashboardType||'BOTH',createdAt:link.createdAt,expiresAt:link.expiresAt,revokedAt:link.revokedAt||null,createdBy:link.createdBy});
  app.get('/api/admin/public-dashboard-links',requireAuth,async(req,res)=>{if(requireAdmin(req,res)!==true)return;try{const user=(req as any).authUser as User;res.json({links:(await listPublicLinks()).filter(link=>scopedRecord(user,{companyId:link.companyId,operationId:link.operationId})).map(publicLinkSummary)});}catch(error){console.error('[public-dashboard] No se pudieron listar enlaces.',error);res.status(502).json({error:'No se pudieron cargar los enlaces.'});}});
  app.post('/api/admin/public-dashboard-links',requireAuth,async(req,res)=>{if(requireAdmin(req,res)!==true)return;try{const user=(req as any).authUser as User,companyId=String(req.body?.companyId||''),campaignId=String(req.body?.campaignId||''),dashboardType=String(req.body?.dashboardType||''),days=Number(req.body?.expiresInDays||90);if(!['QUALITY','D3C'].includes(dashboardType))return res.status(400).json({error:'Selecciona Calidad o D+3C.'});if(![30,90,365].includes(days))return res.status(400).json({error:'Vigencia no válida.'});const repo=await readRepository(),company=repo.companies?.find(item=>item.id===companyId&&item.status==='ACTIVA'),campaign=repo.campaigns.find(item=>item.id===campaignId&&item.status==='ACTIVA'),operation=repo.operations?.find(item=>item.companyId===companyId&&item.campaignId===campaignId&&!item.legacy&&item.status==='ACTIVA');if(!company||!campaign||!operation||!scopedRecord(user,{companyId,operationId:operation.id}))return res.status(404).json({error:'Empresa y campaña activas no encontradas en tu alcance.'});const id=randomBytes(12).toString('hex'),secret=randomBytes(32).toString('base64url'),now=new Date();const link={id,secretHash:createHash('sha256').update(secret).digest('hex'),companyId,campaignId,operationId:operation.id,companyName:company.name,campaignName:campaign.name,dashboardType,createdBy:user.id,createdAt:now.toISOString(),expiresAt:new Date(now.getTime()+days*86400000).toISOString(),revokedAt:null};await savePublicLink(link);res.status(201).json({link:publicLinkSummary(link),path:`/share/dashboard/${id}.${secret}`});}catch(error){console.error('[public-dashboard] No se pudo crear enlace.',error);res.status(502).json({error:'No se pudo crear el enlace público.'});}});
  app.post('/api/admin/public-dashboard-links/:id/revoke',requireAuth,async(req,res)=>{if(requireAdmin(req,res)!==true)return;try{const link=await getPublicLink(String(req.params.id));const user=(req as any).authUser as User;if(!link||!scopedRecord(user,{companyId:link.companyId,operationId:link.operationId}))return res.status(404).json({error:'Enlace no encontrado.'});if(!link.revokedAt){link.revokedAt=new Date().toISOString();await savePublicLink(link);}res.json({link:publicLinkSummary(link)});}catch(error){console.error('[public-dashboard] No se pudo revocar enlace.',error);res.status(502).json({error:'No se pudo revocar el enlace.'});}});
  app.get('/api/public/dashboard/:token',async(req,res)=>{
    res.set({'Cache-Control':'no-store','X-Robots-Tag':'noindex, nofollow','Referrer-Policy':'no-referrer'});
    const match=/^([a-f0-9]{24})\.([A-Za-z0-9_-]{43})$/.exec(String(req.params.token||''));
    if(!match)return res.status(404).json({error:'Enlace no disponible.'});
    try{
      const link=await getPublicLink(match[1]);
      const digest=createHash('sha256').update(match[2]).digest();
      if(!link||link.revokedAt||Date.parse(link.expiresAt)<=Date.now()||!timingSafeEqual(digest,Buffer.from(String(link.secretHash||'').padEnd(64,'0').slice(0,64),'hex')))return res.status(404).json({error:'Enlace no disponible o caducado.'});
      const [repo,source,thresholds]=await Promise.all([supabaseStorage.enabled?supabaseStorage.loadRepository():readRepository(),supabaseStorage.enabled?supabaseStorage.loadEvaluations():adminEvaluationRows(),supabaseStorage.enabled?supabaseStorage.loadPublicDashboardThresholds():Promise.resolve(JSON.parse(String((db.prepare("SELECT payload_json FROM app_state WHERE id='global'").get() as any)?.payload_json||'{}')).config?.priorityThresholds||null)]);
      if(!repo)throw new Error('Repositorio no disponible.');
      const operationIds=new Set((repo.operations||[]).filter(item=>item.companyId===link.companyId&&item.campaignId===link.campaignId&&!item.legacy).map(item=>item.id));
      if(!operationIds.size)return res.status(404).json({error:'Campaña no disponible.'});
      const advisorById=new Map<string,Advisor>(repo.advisors.map(item=>[item.id,item]));
      const userById=new Map<string,User>(repo.users.map(item=>[item.id,item]));
      const advisors=repo.advisors.filter(item=>operationIds.has(item.operationId||'')&&item.campaignId===link.campaignId).map(item=>({id:item.id,name:item.name,status:item.status,supervisorName:userById.get(item.supervisorId)?.name||item.supervisor||'Sin supervisor'}));
      const dashboardType=link.dashboardType==='QUALITY'||link.dashboardType==='D3C'?link.dashboardType:'BOTH';
      const evaluations=source.filter((item:any)=>(dashboardType==='BOTH'||(item.evaluationType==='QUALITY'?'QUALITY':'D3C')===dashboardType)&&(operationIds.has(item.operationId)||(!item.operationId&&item.companyId===link.companyId&&item.campaignId===link.campaignId))).map((item:any)=>{
        const score=item.evaluationType==='QUALITY'?item.technicalScore??item.scoreTotal:item.scoreTotal;
        return {id:item.id,advisorId:item.advisorId,advisorName:advisorById.get(item.advisorId)?.name||item.sourceAdvisorName||'Asesor sin identificar',supervisorName:userById.get(item.supervisorAtEvaluation||item.supervisorId)?.name||advisorById.get(item.advisorId)?.supervisor||'Sin supervisor',date:item.date,evaluationType:item.evaluationType==='QUALITY'?'QUALITY':'D3C',origin:item.origin==='SPEECH_ANALYTICS'?'SPEECH_ANALYTICS':'MANUAL',speechTypification:item.speechTypification||null,score:typeof score==='number'&&Number.isFinite(score)?score:null,scoreConnect:typeof item.scoreConnect==='number'?item.scoreConnect:null,scoreClarify:typeof item.scoreClarify==='number'?item.scoreClarify:null,scoreConvert:typeof item.scoreConvert==='number'?item.scoreConvert:null,items:Array.isArray(item.items)?item.items.filter((criterion:any)=>typeof criterion?.criterionId==='string'&&['CUMPLE','NO_CUMPLE','NO_APLICA'].includes(criterion.compliance)).map((criterion:any)=>({criterionId:criterion.criterionId,compliance:criterion.compliance})):[],validationStatus:item.validationStatus||null,qualityResult:item.qualityResult||null,primaryGap:item.primaryGap||null};
      });
      res.json({dashboardType,thresholds:{highGapMax:Number(thresholds?.highGapMax)||60,expectedMax:Number(thresholds?.expectedMax)||90},company:{id:link.companyId,name:repo.companies?.find(item=>item.id===link.companyId)?.name||link.companyName},campaign:{id:link.campaignId,name:repo.campaigns.find(item=>item.id===link.campaignId)?.name||link.campaignName},generatedAt:new Date().toISOString(),expiresAt:link.expiresAt,advisors,evaluations});
    }catch(error){console.error('[public-dashboard] No se pudo cargar vista pública.',error);res.status(502).json({error:'No se pudo cargar el dashboard. Intenta nuevamente.'});}
  });
  ...
  const sourceBatchDate=limaDate(),sourceBatchId=`sa_${sourceBatchDate.replace(/-/g,'')}_${speechHash(`${campaignName}|${req.body?.fileName}|${Date.now()}`).slice(0,12)}`;
    const allowedTipifications=new Set(['CORTA_LLAMADA','PREFIERE_PREPAGO','NO_ES_TITULAR']);
    for(const source of rows){
      ...
      if(prior&&speechNeedsRepair(prior,source)&&source.status==='REPAIR'){
        if(!allowedTipifications.has(source.speechTypification))return res.status(400).json({error:`Selecciona una tipificación válida para la fila ${source.rowNumber||''}.`});
        ...
        repaired.push(correctMigracionesQualityEvaluation({...prior,items:source.items,speechScore:source.speechScore,scoreTotal:source.speechScore,technicalScore:source.speechScore,speechTypification:source.speechTypification,comments:speechText(source.description),sourceAdvisorDni:sourceDni,sourceAdvisorName,...(advisor?{advisorId:advisor.id,operationId:operation?.id,companyId:company?.id,assignmentId:assignment?.id,supervisorId:advisor.supervisorId||'',supervisorAtEvaluation:advisor.supervisorId||'',advisorResolutionStatus:'RESOLVED',importAlert:undefined}: {importAlert:`El DNI ${sourceDni} no figura en la dotación activa de ${campaignName}. Solicita a Administración crear o regularizar al asesor y luego relaciónalo.`})},directory.campaigns));
  ...
  const saveAlert=async(item:any)=>{if(supabaseStorage.enabled){await supabaseStorage.saveQualityAlert(item);try{persistAlert(item);}catch(error){console.error('[quality-alerts] Caché local no disponible',error);}return;}if(googleStorage.enabled)await googleStorage.saveQualityAlert(item);persistAlert(item);};
  const loadAlerts = async () => { let items=alertRows();if(supabaseStorage.enabled){const remote=await supabaseStorage.loadQualityAlerts(),merged=new Map(remote.map((item:any)=>[item.id,item]));for(const local of items){const current:any=merged.get(local.id);if(!current||String(local.updatedAt||'')>String(current.updatedAt||'')){await supabaseStorage.saveQualityAlert(local);merged.set(local.id,local);}}items=[...merged.values()] as any[];for(const item of items)try{persistAlert(item);}catch(error){console.error('[quality-alerts] Caché local no disponible',error);}}else if(googleStorage.enabled){try{const remote=await googleStorage.loadQualityAlerts();if(remote.length){items=remote;remote.forEach(persistAlert);}}catch{}}return items.map(normalizeAlert); };
  ...
  app.patch('/api/quality-alerts/:id', requireAuth, async (req, res) => {
    ...
    const editable=['title','audioUrl','audioFileName','audioFileSize','audioDurationSeconds','audioMimeType','contactNumber','detail','advisorId','campaignId','validUntil','criticality']; if(qualityManagers.has(user.role)){const changes=Object.fromEntries(editable.filter(k=>body[k]!==undefined).map(k=>[k,body[k]]));const alert={...current,...changes,status:current.status,closedAt:current.closedAt,updatedAt:now};try{await saveAlert(alert);return res.json({alert});}catch{return res.status(502).json({error:'No fue posible actualizar la alerta.'});}}
    ...
    try { await saveAlert(alert); res.json({ alert }); } catch { res.status(502).json({ error:'No fue posible actualizar la alerta.' }); }
  });
  app.delete('/api/quality-alerts/:id',requireAuth,async(req,res)=>{const user=(req as any).authUser as User;if(!qualityManagers.has(user.role))return res.status(403).json({error:'Acceso denegado.'});const current=(await loadAlerts()).find(i=>i.id===req.params.id);if(!current||!scopedRecord(user,current))return res.status(404).json({error:'Alerta no encontrada.'});try{if(supabaseStorage.enabled)await supabaseStorage.deleteQualityAlert(current.id);else if(googleStorage.enabled)await googleStorage.deleteQualityAlert(current.id);db.prepare('DELETE FROM quality_alerts WHERE id=?').run(current.id);res.json({ok:true});}catch{res.status(502).json({error:'No se pudo eliminar la alerta.'});}});
  const memoRows=(advisorId?:string)=>(advisorId?db.prepare('SELECT data_json FROM memorandums WHERE advisor_id=? ORDER BY created_at DESC').all(advisorId):db.prepare('SELECT data_json FROM memorandums ORDER BY created_at DESC').all() as any[]).map((row:any)=>JSON.parse(row.data_json));
  const loadMemos=(advisorId?:string)=>supabaseStorage.enabled?supabaseStorage.loadMemorandums(advisorId):Promise.resolve(memoRows(advisorId));
  const saveMemo=async(item:any)=>{if(supabaseStorage.enabled)await supabaseStorage.saveMemorandum(item);else db.prepare('INSERT INTO memorandums(id,alert_id,advisor_id,status,data_json,created_at,updated_at) VALUES(?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET status=excluded.status,data_json=excluded.data_json,updated_at=excluded.updated_at').run(item.id,item.alertId,item.advisorId,item.status,JSON.stringify(item),item.createdAt,item.updatedAt);};
  const memoVisible=(user:User,item:any)=>user.role==='ASESOR'?user.advisorId===item.advisorId:['ADMINISTRADOR','CONSULTOR','SUPERVISOR'].includes(user.role)&&scopedRecord(user,item);
  app.get('/api/memorandums',requireAuth,async(req,res)=>{const user=(req as any).authUser as User,advisorId=String(req.query.advisorId||'');try{const items=(await loadMemos(advisorId||undefined)).filter((item:any)=>memoVisible(user,item));res.json({memorandums:items});}catch(error){console.error('[memorandums] Error de consulta',error);res.status(502).json({error:'No se pudo cargar el historial de memorándums.'});}});
  const memoDocumentTypes:Record<string,string>={pdf:'application/pdf',doc:'application/msword',docx:'application/vnd.openxmlformats-officedocument.wordprocessingml.document',xls:'application/vnd.ms-excel',xlsx:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',ppt:'application/vnd.ms-powerpoint',pptx:'application/vnd.openxmlformats-officedocument.presentationml.presentation',txt:'text/plain',rtf:'application/rtf',odt:'application/vnd.oasis.opendocument.text',ods:'application/vnd.oasis.opendocument.spreadsheet',odp:'application/vnd.oasis.opendocument.presentation'};
  app.post('/api/memorandums/:id/attachments',requireAuth,parseRawFile,async(req,res)=>{
    const user=(req as any).authUser as User;
    if(user.role!=='ADMINISTRADOR')return res.status(403).json({error:'Solo el administrador puede anexar documentos.'});
    let name='';try{name=decodeURIComponent(req.header('x-file-name')||'');}catch{return res.status(400).json({error:'Nombre de archivo inválido.'});}
    name=name.replace(/[\\/\r\n]/g,'_').trim();
    const extension=name.split('.').pop()?.toLowerCase()||'',mimeType=memoDocumentTypes[extension],data=req.body;
    if(!name||name.length>220||!mimeType||!Buffer.isBuffer(data)||!data.length)return res.status(400).json({error:'Adjunta un documento PDF, Word, Excel, PowerPoint, texto u OpenDocument válido.'});
    if(data.length>35*1024*1024)return res.status(413).json({error:'El archivo supera el límite de 35 MB.'});
    if(extension==='pdf'&&data.subarray(0,5).toString()!=='%PDF-')return res.status(415).json({error:'El contenido no corresponde a un PDF.'});
    if(['docx','xlsx','pptx','odt','ods','odp'].includes(extension)&&data.subarray(0,2).toString()!=='PK')return res.status(415).json({error:'El contenido no corresponde al formato seleccionado.'});
    if(['doc','xls','ppt'].includes(extension)&&!data.subarray(0,8).equals(Buffer.from('d0cf11e0a1b11ae1','hex')))return res.status(415).json({error:'El contenido no corresponde al formato seleccionado.'});
    try{
      const current=(await loadMemos()).find((item:any)=>item.id===req.params.id);
      if(!current||!memoVisible(user,current))return res.status(404).json({error:'Memorándum no encontrado.'});
      if(current.status!=='EMITIDO')return res.status(409).json({error:'No se pueden agregar anexos a un memorándum anulado.'});
      if((current.attachments||[]).length>=10)return res.status(409).json({error:'El memorándum alcanzó el máximo de 10 anexos.'});
      const file=await supabaseFileStorage.uploadFile({name,mimeType,data});
      const attachment={id:file.id,name:file.name,mimeType:file.mimeType,size:file.size,uploadedAt:new Date().toISOString(),uploadedBy:user.id};
      const memorandum={...current,attachments:[...(current.attachments||[]),attachment],updatedAt:attachment.uploadedAt};
      try{await saveMemo(memorandum);}catch(error){await supabaseFileStorage.deleteFile(file.id).catch(()=>{});throw error;}
      res.status(201).json({memorandum});
    }catch(error){console.error('[memorandums] Error al anexar documento',error);res.status(502).json({error:'No fue posible anexar el documento.'});}
  });
  app.get('/api/memorandums/:id/attachments/:fileId',requireAuth,async(req,res)=>{
    const user=(req as any).authUser as User;
    try{
      const current=(await loadMemos()).find((item:any)=>item.id===req.params.id);
      if(!current||!memoVisible(user,current))return res.status(404).json({error:'Documento no encontrado.'});
      const attachment=(current.attachments||[]).find((item:any)=>item.id===req.params.fileId);
      if(!attachment)return res.status(404).json({error:'Documento no encontrado.'});
      const stream=await fileStorageFor(attachment.id).downloadFile(attachment.id);
      res.set({'Content-Type':attachment.mimeType,'Content-Disposition':`attachment; filename="anexo.${attachment.name.split('.').pop()}"; filename*=UTF-8''${encodeURIComponent(attachment.name)}`,'Cache-Control':'private, no-store','X-Content-Type-Options':'nosniff'});
      stream.on('error',()=>res.destroy());stream.pipe(res);
    }catch(error){console.error('[memorandums] Error al descargar documento',error);if(!res.headersSent)res.status(404).json({error:'Documento no encontrado.'});}
  });
  app.post('/api/quality-alerts/:id/memorandums',requireAuth,async(req,res)=>{const user=(req as any).authUser as User;if(user.role!=='ADMINISTRADOR')return res.status(403).json({error:'Solo el administrador puede emitir memorándums.'});try{const alert=(await loadAlerts()).find(item=>item.id===req.params.id);if(!alert||!scopedRecord(user,alert))return res.status(404).json({error:'Alerta no encontrada.'});const existing=(await loadMemos(alert.advisorId)).find((item:any)=>item.alertId===alert.id);if(existing)return res.status(409).json({error:'Esta alerta ya tiene un memorándum.',memorandum:existing});const subject=String(req.body?.subject||'').trim(),content=String(req.body?.content||'').trim(),actionRequired=String(req.body?.actionRequired||'').trim();if(!subject||!content||!actionRequired||subject.length>180||content.length>10000||actionRequired.length>3000)return res.status(400).json({error:'Completa asunto, hechos y acción requerida dentro de los límites permitidos.'});const directory=await readRepository(),advisor=directory.advisors.find(item=>item.id===alert.advisorId);if(!advisor)return res.status(404).json({error:'Asesor no encontrado.'});const campaign=directory.campaigns.find(item=>item.id===alert.campaignId),operation=directory.operations?.find(item=>item.id===alert.operationId),company=directory.companies?.find(item=>item.id===operation?.companyId);const now=new Date().toISOString(),id=`MEM-${now.slice(0,4)}-${randomBytes(4).toString('hex').toUpperCase()}`;const memorandum={id,number:id,alertId:alert.id,advisorId:advisor.id,advisorName:advisor.name,advisorDni:advisor.dni,campaignId:alert.campaignId,campaignName:campaign?.name||'',operationId:alert.operationId,companyId:company?.id,companyName:company?.name||'',alertTitle:alert.title,alertDetail:alert.detail,subject,content,actionRequired,status:'EMITIDO',issuedBy:user.id,issuerName:user.name,createdAt:now,updatedAt:now};await saveMemo(memorandum);res.status(201).json({memorandum});}catch(error:any){if(error?.code==='23505'||String(error?.message||'').includes('UNIQUE'))return res.status(409).json({error:'Esta alerta ya tiene un memorándum.'});console.error('[memorandums] Error de emisión',error);res.status(502).json({error:'No se pudo guardar el memorándum.'});}});
  app.patch('/api/memorandums/:id/annul',requireAuth,async(req,res)=>{const user=(req as any).authUser as User;if(user.role!=='ADMINISTRADOR')return res.status(403).json({error:'Acceso denegado.'});try{const current=(await loadMemos()).find((item:any)=>item.id===req.params.id);if(!current||!memoVisible(user,current))return res.status(404).json({error:'Memorándum no encontrado.'});if(current.status==='ANULADO')return res.status(409).json({error:'El memorándum ya está anulado.'});const reason=String(req.body?.reason||'').trim();if(reason.length<5||reason.length>1000)return res.status(400).json({error:'Indica un motivo de anulación de 5 a 1000 caracteres.'});const memorandum={...current,status:'ANULADO',annulledBy:user.id,annulledAt:new Date().toISOString(),annulReason:reason,updatedAt:new Date().toISOString()};await saveMemo(memorandum);res.json({memorandum});}catch(error){console.error('[memorandums] Error de anulación',error);res.status(502).json({error:'No se pudo anular el memorándum.'});}});
  ...
  app.get('/api/development/assignments', requireAuth, async (req, res) => {
    ...
    if(user.role==='MONITOR'){const ids=new Set(capsuleRows().filter(item=>item.createdByUserId===user.id).map(item=>item.id));assignments=assignments.filter(item=>ids.has(item.capsuleId));}
    if(user.role==='ASESOR')return res.json({assignments:assignments.filter(item=>item.advisorId===user.advisorId)});
    const directory=await readRepository(),evaluationRows=(await adminEvaluationRows()).filter(item=>normalizedValidationStatus(item)==='VALIDATED');
    assignments=assignments.map(item=>{const assignedAt=Date.parse(item.assignedAt),advisorEvaluations=evaluationRows.filter(e=>e.advisorId===item.advisorId&&(e.technicalScore??e.scoreTotal)!==null&&(e.technicalScore??e.scoreTotal)!==undefined&&(e.technicalScore??e.scoreTotal)!=='').map(e=>({...e,_at:Date.parse(`${e.date||''}T${e.time||'00:00'}:00`),_score:Number(e.technicalScore??e.scoreTotal)})).filter(e=>Number.isFinite(e._at)&&Number.isFinite(e._score)).sort((a,b)=>a._at-b._at),before=advisorEvaluations.filter(e=>e._at<=assignedAt).at(-1),after=advisorEvaluations.filter(e=>e._at>assignedAt).at(-1),advisor=directory.advisors.find(a=>a.id===item.advisorId);return{...item,baselineScore:before?._score??null,latestScore:after?._score??null,scoreDelta:before&&after?Math.round((after._score-before._score)*10)/10:null,advisorActive:Boolean(advisor&&advisor.active!==false&&advisor.status!=='INACTIVO')};});
    return res.json({assignments});
  });
  app.post('/api/development/assignments', requireAuth, async (req, res) => {
    if (developmentAdmin(req, res) !== true) return; const body = req.body || {}; const capsuleRow = db.prepare('SELECT data_json FROM development_capsules WHERE id=? AND status=?').get(body.capsuleId, 'PUBLICADA') as any; if (!capsuleRow) return res.status(400).json({ error: 'La cápsula debe estar publicada.' }); const capsule=JSON.parse(capsuleRow.data_json);
    const directory = await readRepository();
    ...
    for (const advisorId of advisorIds) { const advisor=directory.advisors.find(item=>item.id===advisorId);if(!advisor||advisor.active===false||advisor.status==='INACTIVO')continue;const assignment = { id: `asg_${randomBytes(8).toString('hex')}`, capsuleId: body.capsuleId, advisorId, campaignId: advisor.campaignId || body.campaignId || null, groupId: advisor.teamId || body.groupId || null, origin: body.origin || 'Manual', originId: body.originId || null, gap: body.gap || capsule.gap || '', assignedAt: now, dueAt: body.dueAt || capsule.dueAt || null, status: 'PENDIENTE', progress: 0, result: null, attempts: 0, duration: 0, forumPost: null, evidence: null, updatedAt: now }; try { db.prepare('INSERT INTO development_assignments (id,capsule_id,advisor_id,status,data_json,created_at,updated_at) VALUES (?,?,?,?,?,?,?)').run(assignment.id, assignment.capsuleId, advisorId, assignment.status, JSON.stringify(assignment), now, now); created.push(assignment); } catch {} }
  ...
  app.use('/share/dashboard/',(_req,res,next)=>{res.set({'Cache-Control':'no-store','X-Robots-Tag':'noindex, nofollow','Referrer-Policy':'no-referrer'});next();});
  if (!isProduction) {
