...
async function startServer() {
  ...
  app.patch('/api/quality-alerts/:id', requireAuth, async (req, res) => {
    ...
    const editable=['title','audioUrl','contactNumber','detail','advisorId','campaignId','validUntil','criticality']; if(qualityManagers.has(user.role)){const changes=Object.fromEntries(editable.filter(k=>body[k]!==undefined).map(k=>[k,body[k]]));const alert={...current,...changes,status:current.status,closedAt:current.closedAt,updatedAt:now};try{persistAlert(alert);if(googleStorage.enabled)await googleStorage.saveQualityAlert(alert);return res.json({alert});}catch{return res.status(502).json({error:'No fue posible actualizar la alerta.'});}}
