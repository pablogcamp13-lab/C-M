...
async function startServer() {
  ...
  const publicLinkSummary=(link:any)=>({id:link.id,companyId:link.companyId,campaignId:link.campaignId,companyName:link.companyName,campaignName:link.campaignName,createdAt:link.createdAt,expiresAt:link.expiresAt,revokedAt:link.revokedAt||null,createdBy:link.createdBy});
  ...
  app.get('/api/public/dashboard/:token',async(req,res)=>{res.set({'Cache-Control':'no-store','X-Robots-Tag':'noindex, nofollow','Referrer-Policy':'no-referrer'});const match=/^([a-f0-9]{24})\.([A-Za-z0-9_-]{43})$/.exec(String(req.params.token||''));if(!match)return res.status(404).json({error:'Enlace no disponible.'});try{const link=await getPublicLink(match[1]);const digest=createHash('sha256').update(match[2]).digest();if(!link||link.revokedAt||Date.parse(link.expiresAt)<=Date.now()||!timingSafeEqual(digest,Buffer.from(String(link.secretHash||'').padEnd(64,'0').slice(0,64),'hex')))return res.status(404).json({error:'Enlace no disponible o caducado.'});const repo=supabaseStorage.enabled?await supabaseStorage.loadRepository():await readRepository();if(!repo)throw new Error('Repositorio no disponible.');const operationIds=new Set((repo.operations||[]).filter(item=>item.companyId===link.companyId&&item.campaignId===link.campaignId&&!item.legacy).map(item=>item.id));if(!operationIds.size)return res.status(404).json({error:'Campaña no disponible.'});const advisorById=new Map<string,Advisor>(repo.advisors.map(item=>[item.id,item])),userById=new Map<string,User>(repo.users.map(item=>[item.id,item]));const advisors=repo.advisors.filter(item=>operationIds.has(item.operationId||'')&&item.campaignId===link.campaignId).map(item=>({id:item.id,name:item.name,status:item.status,supervisorName:userById.get(item.supervisorId)?.name||item.supervisor||'Sin supervisor'}));const source=supabaseStorage.enabled?await supabaseStorage.loadEvaluations():await adminEvaluationRows();const evaluations=source.filter((item:any)=>operationIds.has(item.operationId)||(!item.operationId&&item.companyId===link.companyId&&item.campaignId===link.campaignId)).map((item:any)=>({id:item.id,advisorId:item.advisorId,advisorName:advisorById.get(item.advisorId)?.name||item.sourceAdvisorName||'Asesor sin identificar',supervisorName:userById.get(item.supervisorAtEvaluation||item.supervisorId)?.name||advisorById.get(item.advisorId)?.supervisor||'Sin supervisor',date:item.date,evaluationType:item.evaluationType==='QUALITY'?'QUALITY':'D3C',origin:item.origin==='SPEECH_ANALYTICS'?'SPEECH_ANALYTICS':'MANUAL',speechTypification:item.speechTypification||null,score:typeof(item.evaluationType==='QUALITY'?item.technicalScore??item.scoreTotal:item.scoreTotal)==='number'?(item.evaluationType==='QUALITY'?item.technicalScore??item.scoreTotal:item.scoreTotal):null,validationStatus:item.validationStatus||null,qualityResult:item.qualityResult||null,primaryGap:item.primaryGap||null}));res.json({company:{id:link.companyId,name:repo.companies?.find(item=>item.id===link.companyId)?.name||link.companyName},campaign:{id:link.campaignId,name:repo.campaigns.find(item=>item.id===link.campaignId)?.name||link.campaignName},generatedAt:new Date().toISOString(),expiresAt:link.expiresAt,advisors,evaluations});}catch(error){console.error('[public-dashboard] No se pudo cargar vista pública.',error);res.status(502).json({error:'No se pudo cargar el dashboard. Intenta nuevamente.'});}});
