...
async function startServer() {
  ...
  const planMetrics=(value:any,ids:string[])=>ids.map(advisorId=>{const row=Array.isArray(value)?value.find((item:any)=>item.advisorId===advisorId):undefined;const initial=row?.sphInitial,updated=row?.sphUpdated,retraining=row?.sphRetraining;const sphInitial=initial===''||initial==null?null:Number(initial),sphUpdated=updated===''||updated==null?null:Number(updated),sphRetraining=retraining===''||retraining==null?null:Number(retraining);if(sphInitial===null||!Number.isFinite(sphInitial)||sphInitial<0||sphUpdated!==null&&(!Number.isFinite(sphUpdated)||sphUpdated<0)||sphRetraining!==null&&(!Number.isFinite(sphRetraining)||sphRetraining<0))throw new Error('Registra SPH válidos para cada asesor.');return{advisorId,sphInitial,sphUpdated,sphRetraining,followUpType:row?.followUpType==='REENTRENAMIENTO'?'REENTRENAMIENTO':'SEGUIMIENTO_FEEDBACK',observations:String(row?.observations||'').trim().slice(0,4000)};});
