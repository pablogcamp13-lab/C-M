...
async function startServer() {
  ...
  const planMetrics=(value:any,ids:string[])=>ids.map(advisorId=>{const row=Array.isArray(value)?value.find((item:any)=>item.advisorId===advisorId):undefined;const initial=row?.sphInitial,updated=row?.sphUpdated;const sphInitial=initial===''||initial==null?null:Number(initial),sphUpdated=updated===''||updated==null?null:Number(updated);if(sphInitial===null||!Number.isFinite(sphInitial)||sphInitial<0||sphUpdated!==null&&(!Number.isFinite(sphUpdated)||sphUpdated<0))throw new Error('Registra un SPH inicial válido para cada asesor; el SPH actualizado puede quedar pendiente.');return{advisorId,sphInitial,sphUpdated,observations:String(row?.observations||'').trim().slice(0,4000)};});
