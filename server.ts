...
const speechPreviewRow = (row: Record<string, unknown>, rowNumber: number) => {
  ...
  // La nomenclatura SA usa el primer bloque numérico de 8 dígitos como DNI.
  const dni = fileName.match(/(?:^|_)(\d{8})(?=_|\.|$)/)?.[1] || fileName.match(/(?<!\d)(\d{8})(?!\d)/)?.[1] || '';
...
async function startServer() {
  ...
  app.post('/api/evaluations/import-speech/preview', requireAuth, parseRawFile, async (req,res) => {
    ...
    try{
      ...
      const rows=sourceRows.map((source,rowIndex)=>{
        ...
        return{...parsed,evaluationId,matchedAdvisorId:advisor?.id||'',matchedAdvisorName:advisor?.name||'',matchedOperationId:operation?.id||'',matchedOperationName:operation?.name||'',matchedCompanyId:company?.id||'',matchedCompanyName:company?.name||'',status:duplicate?'DUPLICATE':advisor?'READY':'WARNING',warning:duplicate?'La evaluación ya fue importada.':advisor?'':'El asesor no se encuentra en rotación.'};
      });
  ...
  app.post('/api/evaluations/import-speech', requireAuth, async (req,res) => {
    ...
    for(const source of rows){
      ...
      const sourceDni=speechText(source.sourceAdvisorDni).replace(/\D/g,''),advisor=context.resolve(sourceDni,speechText(source.sourceAdvisorName)),operation=advisor?.operationId?context.operationById.get(advisor.operationId):undefined,campaign=(operation&&context.campaignById.get(operation.campaignId))||context.campaigns[0],company=operation?context.companyById.get(operation.companyId):undefined;
      const operationId=operation?.id;
      const pendingPersonId=`speech_pending_${speechHash(`${context.key}|${sourceDni||normalizeSpeechText(source.sourceAdvisorName)||source.fileName||externalId}`)}`;
      const advisorId=advisor?.id||pendingPersonId;
      if(!advisor&&!pendingPeople.some(person=>person.id===pendingPersonId))pendingPeople.push({id:pendingPersonId,dni:`PENDING-${speechHash(pendingPersonId).slice(0,12)}`,employeeCode:'',name:speechText(source.sourceAdvisorName)||`Asesor por relacionar ${externalId}`,campaignId:campaign.id,teamId:'',supervisorId:'',status:'INACTIVO',active:false,speechImportPlaceholder:true,sourceAdvisorDni:sourceDni,sourceAdvisorName:speechText(source.sourceAdvisorName)});
